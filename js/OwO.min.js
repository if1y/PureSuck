    ...
    class OwO {
        ...
        _init() {
            ...
            this.body.addEventListener('click', (e) => {
                ...
                const pos = this.target?.selectionStart ?? value.length;
                const hasLeadingSpace = pos > 0 && /\s/.test(value.charAt(pos - 1));
                const text = input ? `${hasLeadingSpace ? '' : ' '}${input} ` : '';
        ...
        _bindLayoutSync() {
            ...
            const scope = this.container.closest('.respond, #cf, .tbox, .CtBoxBar') || this.target.closest('.respond, #cf, .tbox, .CtBoxBar');

            if ('ResizeObserver' in window) {
                this._resizeObserver = new ResizeObserver(() => this.syncLayout());
                if (scope) this._resizeObserver.observe(scope);
                this._resizeObserver.observe(this.container);
                return;
            }

            this.target.addEventListener('input', () => this.syncLayout(), { passive: true });
        }
        ...
        syncLayout() {
            ...
            const formRect = (this.target.closest('.respond, #cf, .tbox') || this.target).getBoundingClientRect();
            const viewportWidth = Math.max(document.documentElement.clientWidth || 0, window.innerWidth || 0);
            const gutter = 12;
            const preferredWidth = Math.min(550, Math.max(260, viewportWidth - gutter * 2));
            const maxWidth = Math.max(220, Math.floor(formRect.right - buttonRect.left));
            const finalWidth = Math.max(220, Math.min(preferredWidth, maxWidth));

            bodyStyle.width = `${finalWidth}px`;

            const overflowRight = bodyRect.right - formRect.right;
            const left = overflowRight > 0 ? Math.min(0, -Math.ceil(overflowRight)) : 0;
            bodyStyle.left = `${left}px`;
            bodyStyle.right = 'auto';
        }
